...
public class EventDataInitializer {
  ...
  private void initEvents() {
    ...
    saveFields(purchaseClick,
        field("approvedAmount", "NUMBER", true, "결제 승인된 최종 금액 (원 단위)"),
        field("productName", "STRING", true, "구매한 상품의 이름"),
        field("productId", "STRING", true, "상품 고유 식별자"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(cartAction,
        field("productName", "STRING", true, "찜/장바구니 대상 상품명"),
        field("productId", "STRING", true, "상품 고유 식별자"),
        field("actionType", "STRING", true, "add / remove 구분값"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(couponReceived,
        field("couponCode", "STRING", true, "발급된 쿠폰의 고유 코드"),
        field("discountAmount", "NUMBER", true, "쿠폰 적용 시 할인되는 금액 (원 단위)"),
        field("expiryDate", "DATE", false, "쿠폰 유효 만료 날짜 — 만료율 분석에 유용"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(couponUsed,
        field("couponCode", "STRING", true, "사용된 쿠폰의 고유 코드"),
        field("discountAmount", "NUMBER", true, "실제 적용된 할인 금액 (원 단위)"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(adClick,
        field("productName", "STRING", true, "광고된 상품명"),
        field("productId", "STRING", true, "광고 상품 고유 식별자"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(searchClick,
        field("searchKeyword", "STRING", true, "사용자가 입력한 검색어"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(pageView,
        field("pageName", "STRING", true, "진입한 페이지의 이름 또는 경로"),
        field("dwellTime", "TIME", true, "페이지 체류시간 (초 단위)"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(productDetailView,
        field("productName", "STRING", true, "조회한 상품명"),
        field("productId", "STRING", true, "상품 고유 식별자"),
        field("dwellTime", "TIME", true, "상세 페이지 체류시간 (초 단위)"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(reviewSubmit,
        field("reviewRating", "NUMBER", true, "사용자가 부여한 별점 (1~5 정수)"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(pointEarned,
        field("earnedPoints", "NUMBER", true, "이번에 적립된 포인트 수"),
        field("earnReason", "STRING", true, "적립 원인 (구매완료 / 리뷰작성 / 출석 등)"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
    ...
    saveFields(pointUsed,
        field("usedPoints", "NUMBER", true, "이번 거래에서 사용된 포인트 수"),
        field("eventTimestamp", "DATETIME", true, "해당 이벤트 발생 시간")
    );
  }
  ...
  private EventField field(String fieldName, String fieldType, boolean isRequired, String description) {
